...
DATA_FILE = "iplbets.json"
RESULTS_FILE = "matchresults.json"
...
def save_data(data, filename):
    os.makedirs(os.path.dirname(filename), exist_ok=True) if os.path.dirname(filename) else ...
...
def calculate_cumulative_scores(data, schedule):
    history_data = [("Match", {p: 0 for p in PLAYERS})]
    ...
    for match in schedule:
        ...
        if m_id in data["results"] and m_id in data["bets"]:
            ...
            history_data.append((match["match"], {k: round(v) for k, v in cumulative.items()}))
...
def send_whatsapp_reminders(missing_players, match_name, time_remaining):
    ...
    for player in missing_players:
        msg_body = f"PoKaBaSaNaTe Premier League 2026 Alert! {player}, you haven't placed your bet for {match_name} yet! You have {time_remaining} left before you get auto-assigned a team."
        try:
            client.messages.create(
                from_=TWILIO_FROM_NUMBER,
                body=msg_body,
                to=PLAYER_WHATSAPP[player]
            )
...
if st.session_state.logged_in_player is None:
    ...
    with tab1:
        if st.session_state.logged_in_player:
            ...
            # CHANGE 1: Dropdown with Match number - e.g. "M1 - RCB vs SRH"
            match_options = [f"{m['id']} - {m['match']}" for m in schedule]
            selected_option = st.selectbox("Select Match", match_options)
            selected_match = next(m for m in schedule if f"{m['id']} - {m['match']}" == selected_option)
            ...
            if is_past_cutoff:
                ...
                st.caption(f"Closes: {cutoff_time.strftime('%d %b %Y, %I:%M %p')} IST")

                if m_id not in data.get("bets", {}):
                    data["bets"] = data.get("bets", {})
                    data["bets"][m_id] = {}
            ...
            # CHANGE 2: Show all players' bets in same tab
            st.subheader("All Players' Bets")
            match_bets = data.get("bets", {}).get(m_id, {})
            bets_display = {p: match_bets.get(p, "No bet") for p in PLAYERS}
            bets_df = pd.DataFrame(list(bets_display.items()), columns=["Player", "Bet"])
            st.table(bets_df)
    ...
    with tab2:
        ...
        if st.session_state.logged_in_player == "Porwal":
            res_match_str = st.selectbox("Select Match to Resolve", [m["match"] for m in schedule], key="resmatch")
            res_match = next(m for m in schedule if m["match"] == res_match_str)
            r_id = res_match["id"]

            if r_id not in data.get("bets", {}):
                st.info("No bets for this match yet.")
            else:
                with st.form(f"resform_{r_id}"):
                    ...
                    if st.form_submit_button("Confirm Result"):
                        ...
                        data["results"][r_id] = {"winner": winner, "timestamp": current_time_ist.isoformat(), "updated_by": "Porwal"}
                        save_data(data, DATA_FILE)
                        save_data(data["results"], RESULTS_FILE)
